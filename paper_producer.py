...
class PaperProducer(object):
    ...
    def download_paper(self, arxiv_id):
        ...
        self.old_paper_results.download_pdf(dirpath=self.old_papers_dir, filename=f"{arxiv_id}.pdf")

    def pdf2text(self):

        self.old_paper_text = extract_text(os.path.join(self.old_papers_dir, f"{arxiv_id}.pdf"))
...
def parse_arguments(parser):
    ...
    parser.add_argument(
        "--arxiv_id", type=str, required=True, help="arxiv paper ID to use to produce a new paper"
    )
